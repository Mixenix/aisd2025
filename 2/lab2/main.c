...
int main(){
	...
	int ang2;
	double p1, p2, p3;
	int inp = input(&nOfChnls, &ang1, &ang2, &p1, &p2, &p3);
	if (inp == EXIT){
		return 0;
	}
	...
	// alg(&toChannels, &toLeader,  nOfChnls, ang1, ang2, p1, p2, p3);
	...
	while (done < nOfAngles){
		for (int i=0; i<nOfChnls; i++){
			if (((double)rand() / RAND_MAX < p1) && (!isFull(&toChannels[i]))){
				for (int j=0; j <nOfAngles; j++){
					if (res[j].res == -10){
						Task task = {res[j].ang, 0};
